...
public class GitForDataSparkCatalog extends SparkCatalog {
  ...
  public SparkTable createTable(
    ...
    if (branch.isPresent()) {
      ...
      try {
        SparkTable created = super.createTable(ident, schema, transforms, withWap);
  ...
  public SparkTable loadTable(Identifier ident) throws NoSuchTableException {
    ...
    try {
      ...
      SparkTable table = super.loadTable(ident);
      if (branch.isPresent() && SessionBranchRefs.ensureFromMain(table.table(), branch.get())) {
        invalidateTable(ident);
