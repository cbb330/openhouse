 ...
 *
 * <p>How it works: 1. Test classes are on classpath via: testImplementation
 * "org.apache.iceberg:...:tests" 2. Gradle resolves ALL transitive dependencies automatically (via
 * our POM fix!) 3. We load and execute test classes programmatically using JUnit 4 4. ServiceLoader
 * injects OpenHouse catalog into Iceberg's @Parameters 5. Tests run against: hadoop, hive, spark,
 * openhouse
 *
...
public class ZeroCopyIcebergTests {
  ...
  public void verifyTestClassIsOnClasspath() {
    ...
    try {
      Class<?> testClass =
          Class.forName("org.apache.iceberg.spark.sql.TestPartitionedWritesToWapBranch");
