...
public class OpenHouseSparkITestProvider implements TestSparkSessionProvider, TestCatalogProvider {
  ...
  static {
    System.out.println("OpenHouseSparkITestProvider: Static Initializer");
    try {
  ...
  public SparkSession createSparkSession(SparkSession.Builder defaultBuilder, HiveConf hiveConf)
      throws Exception {
    System.out.println("DEBUG: createSparkSession invoked.");
    if (hiveConf != null) {
      String newUri = hiveConf.get("hive.metastore.uris");
      System.out.println("DEBUG: HiveConf URI: " + newUri);
      if (newUri != null) {
        Class<?> shimClass = Class.forName(OPENHOUSE_ITEST_CLASS);
        Field uriField = shimClass.getField("hiveMetastoreUri");
        String currentUri = (String) uriField.get(null);

        if (!newUri.equals(currentUri)) {
          System.out.println(
              "DEBUG: Updating Hive Metastore URI from " + currentUri + " to " + newUri);
          uriField.set(null, newUri);
          if (sharedSession != null) {
            System.out.println("DEBUG: Stopping shared session");
            sharedSession.stop();
            sharedSession = null;
          }
          cachedSession = null;
      ...
      for (Map.Entry<String, String> entry : hiveConf) {
        ...
        if ("hive.metastore.uris".equals(entry.getKey())) {
          ...
          if (sharedServerUri != null) {
            session.conf().set("spark.sql.catalog.testhive.uri", sharedServerUri.toString());
          }
